...
def _summarize_truth_efficiency_metrics(
    event_truth_matched_triplet_counts: Sequence[int],
    truth_matched_triplets_total: int,
    selected_truth_matched_triplets: int,
) -> Dict[str, Optional[float] | int]:
    events_total = int(len(event_truth_matched_triplet_counts))
    events_with_truth = int(sum(1 for count in event_truth_matched_triplet_counts if int(count) > 0))
    events_without_truth = int(events_total - events_with_truth)

    efficiency: Optional[float]
    if truth_matched_triplets_total > 0:
        efficiency = float(selected_truth_matched_triplets) / float(truth_matched_triplets_total)
    else:
        efficiency = None

    return {
        "events_total": events_total,
        "events_with_truth_matched_triplets": events_with_truth,
        "events_without_truth_matched_triplets": events_without_truth,
        "truth_matched_triplets_total": int(truth_matched_triplets_total),
        "selected_truth_matched_triplets": int(selected_truth_matched_triplets),
...
def _sorted_candidates(triplets: Iterable[TripletCandidate]) -> List[TripletCandidate]:
    accepted = [t for t in triplets if np.isfinite(t.score)]
    accepted.sort(key=lambda t: (-float(t.score), int(t.i), int(t.j), int(t.k)))
    return accepted


def _threshold_candidates(candidates: Sequence[TripletCandidate], min_score: float) -> List[TripletCandidate]:
    return [t for t in candidates if float(t.score) >= float(min_score)]
...
def _apply_strategy(
    ...
    if strategy == "threshold":
        thresholded = _threshold_candidates(candidates, min_score=min_score)
        return _select_threshold(thresholded, max_top_per_event=max_top_per_event)
...
def run(args: argparse.Namespace) -> None:
    ...
    event_truth_matched_triplet_counts: List[int] = []
    ...
    def flush_event(event_id: int, triplets: Sequence[TripletCandidate]) -> None:
        ...
        nonlocal truth_matched_triplets_total, selected_truth_matched_triplets, event_truth_matched_triplet_counts
        ...
        truth_triplets_in_event = int(sum(1 for triplet in triplets if bool(triplet.is_truth)))
        truth_matched_triplets_total += truth_triplets_in_event
        event_truth_matched_triplet_counts.append(truth_triplets_in_event)
            ...
            selected_buffer.append_row(
                {
                    ...
                    "is_truth": int(bool(triplet.is_truth)),
                }
            )
            selected_rows_total += 1
            selected_truth_matched_triplets += int(bool(triplet.is_truth))
        ...
        for batch in scanner.to_batches():
            ...
            for idx in range(n_rows):
                ...
                row = TripletCandidate(
                    ...
                    is_truth=bool(payload["is_truth"][idx]),
    ...
    truth_efficiency_metrics = _summarize_truth_efficiency_metrics(
        event_truth_matched_triplet_counts=event_truth_matched_triplet_counts,
        truth_matched_triplets_total=truth_matched_triplets_total,
        selected_truth_matched_triplets=selected_truth_matched_triplets,
    )
    ...
    report = {
        ...
        "truth_matched_triplets_total": int(truth_matched_triplets_total),
        "selected_truth_matched_triplets": int(selected_truth_matched_triplets),
        "triplet_reconstruction_efficiency": truth_efficiency_metrics["triplet_reconstruction_efficiency"],
        "events_with_truth_matched_triplets": int(truth_efficiency_metrics["events_with_truth_matched_triplets"]),
        "events_without_truth_matched_triplets": int(truth_efficiency_metrics["events_without_truth_matched_triplets"]),
...
def register_subparser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Minimum score cut used by --strategy threshold (ignored by other strategies)",
    )
    parser.add_argument(
        "--max-top-per-event",
        type=int,
